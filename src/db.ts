...
// UPDATED: Now remembers the cooked weight
export interface SavedRecipe {
  id?: number;
  name: string;
  ingredients: any[]; 
  defaultCookedWeight?: number; // <--- NEW FIELD
}
...
class MacroPrepDB extends Dexie {
  ...
  constructor() {
    super("MacroPrepDB");
    // Version 9 adds defaultCookedWeight logic
...
// --- 3. THE SEEDER (Clean start with PFC data) ---

export async function seedDatabase() {
  const count = await db.ingredients.count();
  
 if (count === 0) {
    console.log("Seeding Extensive Culinary Library...");
    await db.ingredients.bulkAdd([
      ...
      { name: 'Coconut Oil', calories: 862, protein: 0, carbs: 0, fat: 100, category: 'fat', measure: 'g' }, // usually solid
      ...
      { name: 'Almonds', calories: 579, protein: 21, carbs: 22, fat: 50, category: 'fat', measure: 'g' },
